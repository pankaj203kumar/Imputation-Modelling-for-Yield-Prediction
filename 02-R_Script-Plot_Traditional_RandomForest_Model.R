# THIS CODE RUNS PLOT BASED RANDOM FOREST IMPUTATION MODEL WITH TRADITIONAL (LOOCV-LEAVE ONE OUT CROSS VALIDATION) APPROACH
# -----------------------------------------------
...
# DEVELOP THE TRADITIONAL RANDOM FOREST MODEL
set.seed(1)
start.time = Sys.time()

metrics_lidar <- data.frame(cbind(metrics_file[, 11], metrics_file[, 12:160])) # COMBINE AGE METRIC WITH THE LIDAR METRICS
colnames(metrics_lidar)[1] <- "age" # PROVIDE COLUMN NAME TO THE AGE METRIC
metrics_n_lidar <- data.frame(cbind(metrics_file[, 2], metrics_lidar)) # COMBINE STOCKING METRIC WITH THE LIDAR METRICS
colnames(metrics_n_lidar)[1] <- "n" # PROVIDE COLUMN NAME TO THE STOCKING METRIC
...
metrics_n <- metrics_n_lidar$n # STOCKING METRIC
...
{
  metrics_n_lidar_in <- metrics_n_lidar[-c(m),] # ONE STOCKING BASED PLOT SAMPLE IS LEFT-OUT DURING EACH ITERATION
  metrics_n_rf <- randomForest(n~., data = metrics_n_lidar_in, ntree = 100, mtry = floor(ncol(metrics_n_lidar_in)/3),
                               importance = TRUE, na.action = na.omit) # RANDOM FOREST MODEL IS DEVELOPED FROM REST OF THE PLOT SAMPLES
  metrics_n_imp_out <- predict(metrics_n_rf, metrics_lidar[m,]) # STOCKING METRIC FOR THE LEFT-OUT SAMPLE IS IMPUTED FROM THE MODEL
  metrics_n_imp <- rbind(metrics_n_imp, metrics_n_imp_out) #STOCKING METRIC FOR EACH LEFT-OUT SAMPLE IS COMBINED
...
set.seed(2)
metrics_n_nrmse <- round(nrmse(metrics_n_imp, metrics_n, na.rm = TRUE, norm = "maxmin"), 3)
...
metrics_n_pbias <- round(pbias(metrics_n_imp, metrics_n, na.rm = TRUE), 3)
...
metrics_GI_imp <- cbind(metrics_n_imp, metrics_ba_imp, metrics_h_imp,
                            metrics_vs_imp, metrics_trv_imp, metrics_saw_imp,
                            metrics_industrial_imp, metrics_chip_imp, metrics_pulp_imp,
                            metrics_n, metrics_ba, metrics_h,
                            metrics_vs, metrics_trv, metrics_saw,
                            metrics_industrial, metrics_chip, metrics_pulp)

colnames(metrics_GI_imp)[1] <- "n"
colnames(metrics_GI_imp)[2] <- "ba"
colnames(metrics_GI_imp)[3] <- "h"
colnames(metrics_GI_imp)[4] <- "vs"
colnames(metrics_GI_imp)[5] <- "trv"
colnames(metrics_GI_imp)[6] <- "saw"
colnames(metrics_GI_imp)[7] <- "industrial"
colnames(metrics_GI_imp)[8] <- "chip"
colnames(metrics_GI_imp)[9] <- "pulp"
colnames(metrics_GI_imp)[10] <- "n.o"
colnames(metrics_GI_imp)[11] <- "ba.o"
colnames(metrics_GI_imp)[12] <- "h.o"
colnames(metrics_GI_imp)[13] <- "vs.o"
colnames(metrics_GI_imp)[14] <- "trv.o"
colnames(metrics_GI_imp)[15] <- "saw.o"
colnames(metrics_GI_imp)[16] <- "industrial.o"
colnames(metrics_GI_imp)[17] <- "chip.o"
colnames(metrics_GI_imp)[18] <- "pulp.o"
