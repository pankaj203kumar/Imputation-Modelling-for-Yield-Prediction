# THIS CODE ESTIMATES THE PLOT BASED VOXEL METRICS FROM HEIGHT NORMALIZED LIDAR POINT CLOUD DATA
# -----------------------------------------------
...
lidar_input_file_path = "SET INPUT FOLDER WHERE PLOT BASED HEIGHT NORMALIZED LIDAR FILES ARE LOCATED"
...
lidar_file_list_plotid = list.files(path = lidar_input_file_path, pattern = "\\.laz$") # LISTS PLOT_IDS ASSOCIATED WITH THE LIDAR FILES
...
for (m in 1:length(lidar_file_list))
{
  ...
  # DEVELOP VOXELS TO GENERATE BIOMASS BASED VOXEL METRICS
...
# WRITE OUTPUT PLOT BASED VOXEL METRICS FILE IN .XLSX FORMAT
...
saveWorkbook(voxel_metrics_file,
             "WRITE FOLDER PATH AND OUTPUT PLOT BASED VOXEL METRICS FILE IN .XLSX FORMAT",
             overwrite = TRUE)
