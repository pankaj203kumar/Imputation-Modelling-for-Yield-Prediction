REM THIS CODE GENERATES STANDARD METRICS FROM INDIVIDUAL TREE BASED HEIGHT NORMALIZED LIDAR FILES

@echo off

set Lastools_Directory=C:\LAStools\bin\
set Input_Files_Directory="Input File Path" REM INPUT LIDAR FILES IN .LAZ FORMAT
set Input_Tree_Boundary_File="Input Polygon Shapefile Name" REM INPUT BOUNDARY FILE OF INDIVIDUAL TREES IN .SHP FORMAT
...
-merged ^ REM METRICS FOR EACH INDIVIDUAL TREES ARE MERGED
-lop %Input_Tree_Boundary_File% ^
Tree_ID ^ REM IDS OF INDIVIDUAL TREES
...
-p 1 5 10 25 50 75 90 95 99 ^ REM PERCENTILES (1%,5%,10%,25%,50%,75%,90%,95%,99%) OF POINTS HEIGHT ABOVE THE CUTOFF
-b 10 20 30 40 50 60 70 80 90 ^ REM BICENTILES (10%,20%,30%,40%,50%,60%,70%,80%,90%) OF POINTS HEIGHT ABOVE THE CUTOFF
-c 10 20 30 40 50 ^ REM NUMBER OF POINTS WITH HEIGHT IN BETWEEN 10-20,20-30,30-40,40-50m
-d 10 20 30 40 50 ^ REM DENSITY OF POINTS WITH HEIGHT IN BETWEEN 10-20,20-30,30-40,40-50m
...
-int_p 1 5 10 25 50 75 90 95 99 ^ REM PERCENTILES (1%,5%,10%,25%,50%,75%,90%,95%,99%) OF POINTS INTENSITY ABOVE THE CUTOFF
-int_c 10 20 30 40 50 ^ REM NUMBER OF POINTS WITH INTENSITY IN BETWEEN 10-20,20-30,30-40,40-50m HEIGHTS
-int_d 10 20 30 40 50 ^ REM DENSITY OF POINTS WITH INTENSITY IN BETWEEN 10-20,20-30,30-40,40-50m HEIGHTS
-ocsv ^
